...
train -c -m resnet -psw animals/resnet_$(get_best_model -m resnet -po animals).pth -pd segmentation_from_network -4D -bs 32 -ne 60 -lr 0.0005 -po segmentation_from_network_start_animals
...
train -c -m densenet -psw animals/densenet_$(get_best_model -m densenet -po animals).pth -pd segmentation_from_network -4D -bs 32 -ne 60 -lr 0.0005 -po segmentation_from_network_start_animals
...
train -c -m vgg -psw animals/vgg_$(get_best_model -m vgg -po animals).pth -pd segmentation_from_network -4D -bs 32 -ne 60 -lr 0.0005 -po segmentation_from_network_start_animals
